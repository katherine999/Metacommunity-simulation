...
class patch():
    def __init__(self, patch_name, location, asexual_birth_rate, sexual_birth_rate):
    ...
    def patch_disp_within_from_offsprings_pool(self, disp_within_rate, counter):
        ...
        for h_id, h_object in self.set.items():
            ...
            disp_within_pool = random.sample(self.get_disp_within_offsprings_pool(h_object), int(len(self.get_disp_within_offsprings_pool(h_object))*disp_within_rate)) # 斑块内非本地生境后代迁入个体对象列表
    ...
    def asex_reproduce_mutate_for_dispersal_within_patch(self, mutation_rate, pheno_var_ls, disp_within_rate, counter):
        ...
        for h_id, h_object in self.set.items():
            ...
            offsprings_expection_num = int(np.around(len(asex_parent_pos_ls) * asexual_birth_rate * disp_within_rate)) # 斑块内非本地生境无性生殖后代迁入个体数期望值
    ...
    def sex_reproduce_mutate_for_dispersal_within_patch(self, mutation_rate, pheno_var_ls, disp_within_rate):
        ...
        for h_id, h_object in self.set.items():
            ...
            offsprings_expection_num = int(np.around(len(sex_pairwise_parents_pos_ls) * sexual_birth_rate * disp_within_rate)) # 斑块内非本地有性生殖后代迁入个体数期望值
    ...
    def mixed_reproduce_mutate_for_dispersal_within_patch(self, mutation_rate, pheno_var_ls, disp_within_rate):
        ...
        for h_id, h_object in self.set.items():
            ...
            asex_offs_expectation_num = int(np.around(len(mixed_asex_parent_pos_ls) * asexual_birth_rate * disp_within_rate)) # 斑块内非本地生境无性生殖后代迁入个体数期望值
            sex_offs_expectation_num = int(np.around(len(mixed_sex_pairwise_parents_pos_ls) * sexual_birth_rate * disp_within_rate)) # 斑块内非本地有性生殖后代迁入个体数期望值
    ...
    def patch_asexual_birth_germinate(self, mutation_rate, pheno_var_ls):
        asexual_birth_rate = self.asexual_birth_rate
        for h_id, h_object in self.set.items():
            h_object.hab_asexual_reprodece_germinate(asexual_birth_rate, mutation_rate, pheno_var_ls)
        return 0
...
class metacommunity():
    ...
    def meta_distribution_data_all_time_to_csv(self, dis_data_all_time, all_time_step, file_name):
        ''' '''
        ...
        index = ['initialization']+['time_step%d'%i for i in range(all_time_step)]
...
#####################################################################################################
    ...
    def dyn_pro_striner_tree_network(self, graph_object, terminal_nodes_ls):
        ...
        new_ad_mat = m1.ad_mat_exchange(graph_object, terminal_nodes_ls)
    ...
    def steiner_tree(self, x_range, y_range):
        ...
        ans, path_ls = m1.dyn_pro_striner_tree_network(graph_object=G, terminal_nodes_ls=list(full_con_map.nodes))
...
##################### colonization from mainland species pool #####################
    def colonize_from_propagules_rains(self, species_pool_obj, reproduce_mode, propagules_rain_num):
        ...
        counter = 0
        for indi_object, empty_site_pos in zip(propagules_rains_ls, meta_empty_sites_ls):
...
###################################################################################
    ...
    def immigrant_matrix_to_patch_empty_sites(self, total_disp_among_rate, disp_kernal, graph_object):
        '''
    ...
    def meta_disp_among_patches_from_offsprings_pool(self, total_disp_among_rate, disp_kernal, graph_object):
        ...
        immigrants_matrix = self.immigrant_matrix_to_patch_empty_sites(total_disp_among_rate, disp_kernal, graph_object)
...
if __name__ == '__main__':
    ...
    species_2_phenotype_ls = [(np.around(j, 1), np.around(i, 1)) for i in np.arange(0.0, 1.0, 0.1) for j in np.arange(0.0, 1.0, 0.1)] # (index+1) indicates species_id
    standar_species_object_ls = [species(species_id='sp%d'%(i+1), traits_num=traits_num, pheno_names_ls=pheno_names_ls, mean_pheno_val_ls=(species_2_phenotype_ls[i]), pheno_var_ls=(0.025, 0.025), geno_len_ls=(20, 20)) for i in range(species_num)]
    ...
    logging.basicConfig(filename='mylog3.log', format='%(asctime)s %(message)s', level=logging.INFO)
    

    for i in range(0, patch_num):
        patch_name = 'patch%s'%str(i+1)
        location = patch_location_ls[i]
        p = patch(patch_name, location, asexual_birth_rate, sexual_birth_rate)
    ...
    #empty = m1.empty_graph()
    full = m1.full_con_map()
    #mini = m1.mini_span_tree() 
    #tsp = m1.dyn_pro_traveling_salesman_network()
    #paul_revere_map = m1.paul_revere_network()
    #hier1 = m1.one_center_network()
    #hier2 = m1.central_hierachical_network()
    #steiner, pos_steiner = m1.steiner_tree(metacommunity_x_range, metacommunity_y_range)

    #m1.show_meta_map(empty, title = 'isolated patches')
    m1.show_meta_map(full, title = 'full connection network')
    #m1.show_meta_map(mini, title = 'mini spanning tree')
    #m1.show_meta_map(paul_revere_map, title = 'paul revere network')
    #m1.show_meta_map(tsp, title = 'travelling salesman network')
    #m1.show_meta_map(hier1, title = 'hierachcial network 1')
    #m1.show_meta_map(hier2, title = 'hierachical network 2')
    #m1.show_meta_map(steiner, title = 'steiner tree', pos = pos_steiner)
    ...
    meta_sp_dis_all_time = m1.get_meta_microsites_individuals_sp_id_values()
    meta_micro_phenotype_all_time = m1.get_meta_microsites_individuals_phenotype_values(trait_name='micro_phenotype')
    meta_macro_phenotype_all_time = m1.get_meta_microsites_individuals_phenotype_values(trait_name='macro_phenotype')
    ...
    for time_step in range(all_time_step):
        ...
        m1.meta_asexual_reproduce_mutate_and_dispersal_among_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), total_disp_among_rate=0.1, disp_kernal=2, graph_object=mini)
        ...
        m1.show_meta_individual_num()
        m1.meta_dead_selection(base_dead_rate=0.1, fitness_wid=0.5)
        m1.meta_sexual_reproduce_mutate_and_dispersal_among_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), total_disp_among_rate=0.1, disp_kernal=3, graph_object=mini)
        ...
        log_info = m1.show_meta_individual_num()
        logging.info(log_info)
        log_info = m1.meta_dead_selection(base_dead_rate=0.1, fitness_wid=0.5)
        logging.info('Dead selection process done! \n' + log_info)
        log_info = m1.colonize_from_propagules_rains(species_pool_obj=mainland, reproduce_mode='sexual', propagules_rain_num=100)
        logging.info('Colonizing process under propagules rains done! \n' + log_info)
        logging.info(m1.meta_mixed_asex_and_sex_parents_num())
        log_info = m1.meta_mixed_reproduce_mutate_and_dispersal_among_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), total_disp_among_rate=0.1, disp_kernal=1, graph_object=full)
        logging.info('Dispersal among patches process done! \n' + log_info)
        log_info = m1.meta_mixed_birth_disp_within_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), disp_within_rate=0.1)
        logging.info('Dispersal within patch process done! \n' + log_info)
        log_info = m1.meta_mixed_birth_mutate_germinate(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025))
        ...
        m1.meta_disp_among_patches_from_offsprings_pool(total_disp_among_rate=0.1, disp_kernal=2, graph_object=mini)
        m1.meta_disp_within_patch_from_offsprings_pool(disp_within_rate=0.1)
        m1.meta_germinate_from_offsprings_pool()
        '''
        
        meta_sp_dis_all_time = np.vstack((meta_sp_dis_all_time, m1.get_meta_microsites_individuals_sp_id_values()))
        meta_micro_phenotype_all_time = np.vstack((meta_micro_phenotype_all_time, m1.get_meta_microsites_individuals_phenotype_values(trait_name='micro_phenotype')))
        meta_macro_phenotype_all_time = np.vstack((meta_macro_phenotype_all_time, m1.get_meta_microsites_individuals_phenotype_values(trait_name='macro_phenotype')))
        
        d2 = time.time()
        logging.info("程序运行时间：%.8s s" % (d2-d1) + '\n') 
        
    endtime = time.time()
    dtime = endtime - starttime
    logging.info("一次模拟运行时间：%.8s s" % dtime) 
    
    m1.meta_distribution_data_all_time_to_csv(dis_data_all_time=meta_sp_dis_all_time, all_time_step=all_time_step, file_name='meta_species_distribution_all_time.csv')
    m1.meta_distribution_data_all_time_to_csv(dis_data_all_time=meta_micro_phenotype_all_time, all_time_step=all_time_step, file_name='meta_micro_phenotype_all_time.csv')
    m1.meta_distribution_data_all_time_to_csv(dis_data_all_time=meta_macro_phenotype_all_time, all_time_step=all_time_step, file_name='meta_macro_phenotype_all_time.csv')
    ...
    m1.show_meta_species_distribution(cmap, 'example.jpg')
    m1.show_meta_map(mini)
    '''
