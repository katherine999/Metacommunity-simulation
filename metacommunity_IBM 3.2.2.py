...
class patch():
    ...
    def get_patch_microsites_optimum_sp_id_value_array(self, species_2_phenotype_ls):
        ''''''
        ...
        for h_id, h_object in self.set.items():
            ...
            hab_sp_id_val = species_2_phenotype_ls.index(mean_env_tuple)+1
            hab_sp_id_val_array = np.ones(hab_len*hab_wid, dtype=int)*hab_sp_id_val
...
class metacommunity():
    ...
    def get_meta_microsites_optimum_sp_id_val(self, species_2_phenotype_ls):
        ''' '''
        meta_optimum_sp_id_val_dis = np.array([])
        for patch_id, patch_object in self.set.items():
            patch_optimum_sp_id_val_dis = patch_object.get_patch_microsites_optimum_sp_id_value_array(species_2_phenotype_ls)
...
def main():
    ...
    empty = calculate_graph_object(meta_object=empty_metacommunity, graph_algorithm='isolation')
    full = calculate_graph_object(meta_object=empty_metacommunity, graph_algorithm='full_connection')
    mini = calculate_graph_object(meta_object=empty_metacommunity, graph_algorithm='minimum_spanning_tree')
    tsp = calculate_graph_object(meta_object=empty_metacommunity, graph_algorithm='travelling salesman problem')
    paul = calculate_graph_object(meta_object=empty_metacommunity, graph_algorithm='paul_revere')
    one_center = calculate_graph_object(meta_object=empty_metacommunity, graph_algorithm='one_center_network')
    hierachy = calculate_graph_object(meta_object=empty_metacommunity, graph_algorithm='hierachical_network')
    steiner = calculate_graph_object(meta_object=empty_metacommunity, graph_algorithm='steiner_tree', metacommunity_x_range=metacommunity_x_range, metacommunity_y_range=metacommunity_y_range)
    
    empty_read = read_graph_object_gpickle(empty_metacommunity, 'isolation.gpickle')
    full_read = read_graph_object_gpickle(empty_metacommunity, 'full_connection.gpickle')
    mini_read = read_graph_object_gpickle(empty_metacommunity, 'minimum_spanning_tree.gpickle')
    tsp_read = read_graph_object_gpickle(empty_metacommunity, 'travelling salesman problem.gpickle')
    paul_read = read_graph_object_gpickle(empty_metacommunity, 'paul_revere.gpickle')
    one_center_read = read_graph_object_gpickle(empty_metacommunity, 'one_center_network.gpickle')
    hierachy_read = read_graph_object_gpickle(empty_metacommunity, 'hierachical_network.gpickle')
    steiner_read = read_graph_object_gpickle(empty_metacommunity, 'steiner_tree.gpickle')
    ...
    for rep in range(repeat_times):
        ...
        meta_sp_dis_all_time = meta.get_meta_microsites_optimum_sp_id_val(species_2_phenotype_ls)
        ...
        for time_step in range(all_time_step):
            d1 = time.time()
            if time_step%100==0: print('time_step%d'%time_step)
            ...
            log_info = meta.colonize_from_propagules_rains(species_pool_obj=mainland, reproduce_mode='asexual', propagules_rain_num=100)
            logging.info('Colonizing process under propagules rains done! \n' + log_info)
            log_info = meta.meta_asexual_reproduce_mutate_and_dispersal_among_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), total_disp_among_rate=0.1, disp_kernal=2, graph_object=full)
            logging.info('Dispersal among patches process done! \n' + log_info)
            log_info = meta.meta_asexual_birth_disp_within_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), disp_within_rate=0.11)
            logging.info('Dispersal within patch process done! \n' + log_info)
            log_info = meta.meta_asexual_birth_mutate_germinate(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025))
            ...
            log_info = meta.meta_mixed_reproduce_mutate_and_dispersal_among_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), total_disp_among_rate=0.1, disp_kernal=1, graph_object=full)
            logging.info('Dispersal among patches process done! \n' + log_info)
            log_info = meta.meta_mixed_birth_disp_within_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), disp_within_rate=0.11)
            logging.info('Dispersal within patch process done! \n' + log_info)
            log_info = meta.meta_mixed_birth_mutate_germinate(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025))
            logging.info('Local birth and germination process done! \n' + log_info)
            '''
        ...
        meta.meta_distribution_data_all_time_to_csv_gz(dis_data_all_time=meta_micro_phenotype_all_time, first_row_index_name='micro_environment_values', all_time_step=all_time_step, file_name='rep=%dmeta_micro_phenotype_all_time.gz'%(rep))
        meta.meta_distribution_data_all_time_to_csv_gz(dis_data_all_time=meta_macro_phenotype_all_time, first_row_index_name='macro_environment_values', all_time_step=all_time_step, file_name='rep=%dmeta_macro_phenotype_all_time.gz'%(rep))
