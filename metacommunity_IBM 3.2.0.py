...
class patch():
    def __init__(self, patch_name, location, birth_rate, asexual_birth_rate, sexual_birth_rate):
    ...
    def mixed_reproduce_mutate_for_dispersal_among_patches(self, mutation_rate, pheno_var_ls, patch_offs_num):
        ...
        if patch_offs_num == 0 or (patch_asexual_mixed_parents_num + patch_sexual_mixed_pairwise_parents_num) == 0:
            return patch_disp_among_pool
        else:
            ...
            for h_id, h_object in self.set.items():
                if patch_asexual_mixed_parents_num != 0: 
                    hab_asexual_mixed_offs_num = int(patch_asexual_mixed_offs_num * (h_object.hab_mixed_asexual_parent_num()/patch_asexual_mixed_parents_num))
                ...
                if patch_sexual_mixed_pairwise_parents_num != 0:
                    hab_sexual_mixed_offs_num = int(patch_sexual_mixed_offs_num * (h_object.hab_mixed_sexual_pairwse_parents_num()/patch_sexual_mixed_pairwise_parents_num))
    ...
    def mixed_reproduce_mutate_for_dispersal_within_patch(self, mutation_rate, pheno_var_ls, disp_within_rate):
        ...
        for h_id, h_object in self.set.items():
            ...
            mixed_asex_parent_pos_ls = self.get_disp_within_mixed_asex_parent_pos_ls(h_object)
            mixed_sex_pairwise_parents_pos_ls = self.get_disp_within_mixed_sex_pairwise_parents_pos_ls(h_object) 
            asex_offs_expectation_num = int(len(mixed_asex_parent_pos_ls) * asexual_birth_rate)
            sex_offs_expectation_num = int(len(mixed_sex_pairwise_parents_pos_ls) * sexual_birth_rate)
            
            if (asex_offs_expectation_num + sex_offs_expectation_num) > disp_within_empty_site_num:
...
##################### colonization from mainland species pool #####################
    def colonize_from_propagules_rains(self, species_pool_obj, reproduce_mode, propagules_rain_num):
        ...
        indi_num = self.get_meta_individual_num()
        empty_sites_num = self.show_meta_empty_sites_num()
        print('there are %d individuals colonizing the metacommunity from mainland; there are %d individuals in the metacommunity; there are %d empty sites in the metacommunity'%(counter, indi_num, empty_sites_num))
        return 0
...
###################################################################################
    ...
    def emigrant_matrix_expectation_mixed(self, disp_kernal, graph_object):
        ''''''
        patch_offs_num_matrix = np.mat(np.zeros((self.patch_num, self.patch_num)))
        index = 0
        for patch_id in self.meta_map.nodes():
    ...
    def meta_mixed_asex_and_sex_parents_num(self):
        ...
        print('there are %d asexual parents in the metacommunity; there are %d sexual parents in the metacommunity'%(asex_num, sex_num))
        return 0
    ...
    def meta_mixed_reproduce_mutate_and_dispersal_among_patches(self, mutation_rate, pheno_var_ls, disp_kernal, graph_object):
        emigrants_matrix = self.emigrant_matrix_expectation_mixed(disp_kernal, graph_object)
        immigrants_matrix = self.immigrant_matrix_to_patch_empty_sites(disp_kernal, graph_object)
        migrants_matrix = np.minimum(emigrants_matrix, immigrants_matrix)
        counter = 0
        for j in range(len(self.meta_map.nodes())):
...
if __name__ == '__main__':
    ...
    for i in range(0, patch_num):
        ...
        p = patch(patch_name, location, birth_rate, asexual_birth_rate, sexual_birth_rate)
    ...
    starttime = time.time()
    for timp_step in range(all_time_step):
        d1 = time.time()
        ...
        m1.show_meta_individual_num()
        m1.colonize_from_propagules_rains(species_pool_obj=mainland, reproduce_mode='sexual', propagules_rain_num=100)
        m1.meta_dead_selection(base_dead_rate=0.1, fitness_wid=0.5)
        m1.meta_mixed_asex_and_sex_parents_num()
        m1.meta_mixed_reproduce_mutate_and_dispersal_among_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), disp_kernal=0.1, graph_object=full)
        m1.meta_mixed_birth_disp_within_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), disp_within_rate=0.1)
        m1.meta_mixed_birth_mutate_germinate(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025))
