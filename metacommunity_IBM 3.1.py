...
class habitat():
    def __init__(self, hab_name, num_env_types, env_types_name, mean_env_ls, var_env_ls, length, width):
        ...
        self.reproduction_mode_threhold = 0.89
    ...
    def hab_mixed_reproduce_germinate(self, asexual_birth_rate, sexual_birth_rate, mutation_rate, pheno_var_ls):
        ...
        asex_offs_expectation_num = int(self.hab_mixed_asexual_parent_num() * asexual_birth_rate)
        sex_offs_expectation_num = int(self.hab_mixed_sexual_pairwse_parents_num() * sexual_birth_rate)
        
        if empty_sites_num < asex_offs_expectation_num + sex_offs_expectation_num:
            asex_num = int(empty_sites_num * asex_offs_expectation_num/(asex_offs_expectation_num + sex_offs_expectation_num))
            sex_num = int(empty_sites_num * sex_offs_expectation_num/(asex_offs_expectation_num + sex_offs_expectation_num))
...
class patch():
    def __init__(self, patch_name, location, birth_rate):
        ...
        self.asexual_birth_rate = 0.5 # to be improved
        self.sexual_birth_rate = 1    # to be improved
    ...
    def mixed_reproduce_mutate_for_dispersal_among_patches(self, mutation_rate, pheno_var_ls, patch_offs_num):
        ...
        if patch_offs_num == 0 or (patch_asexual_mixed_parents_num + patch_sexual_mixed_pairwise_parents_num) == 0:
            ...
            patch_asexual_mixed_offs_num = int(patch_offs_num * (patch_asexual_mixed_parents_num * self.asexual_birth_rate)/(patch_asexual_mixed_parents_num * self.asexual_birth_rate + patch_sexual_mixed_pairwise_parents_num * self.sexual_birth_rate))
            patch_sexual_mixed_offs_num = int(patch_offs_num * (patch_sexual_mixed_pairwise_parents_num * self.sexual_birth_rate)/(patch_asexual_mixed_parents_num * self.asexual_birth_rate + patch_sexual_mixed_pairwise_parents_num * self.sexual_birth_rate))
    ...
    def mixed_reproduce_mutate_for_dispersal_within_patch(self, mutation_rate, pheno_var_ls, disp_within_rate):
        ...
        for h_id, h_object in self.set.items():
            ...
            if (asex_offs_expectation_num + sex_offs_expectation_num) > disp_within_empty_site_num:
                asex_disp_num = int(disp_within_empty_site_num * asex_offs_expectation_num/(asex_offs_expectation_num + sex_offs_expectation_num))
                sex_disp_num = int(disp_within_empty_site_num * sex_offs_expectation_num/(asex_offs_expectation_num + sex_offs_expectation_num))
...
if __name__ == '__main__':
    ...
    for timp_step in range(1):
        ...
        m1.show_meta_individual_num()
        m1.meta_dead_selection(base_dead_rate=0.1, fitness_wid=0.5)
        m1.meta_mixed_reproduce_mutate_and_dispersal_among_patches(mutation_rate=0.00001, pheno_var_ls=(0.025, 0.025), disp_kernal=3, graph_object=mini)
